Public Class Frtb_cgptscl
    Private Sub Frtb_cgptscl_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Me.Icon = G_icon : Me.BackgroundImage = G_backimg
        GS_qhjd(Me) '回车键切换焦点，备注除外
        showComboList(Me.tb_cgptscl_wancheng)
    ...
    Private Sub SSSQD_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SSSQD.Click
        If MsgBox("确定更新数据吗？", 1, "提示") <> 1 Then
            Exit Sub
        End If
        'If Me.tb_cgptscl_wancheng.Text = "完成" Then
        '    MsgBox("已经完成不能修改！")
        '    Me.Close()
        '    Exit Sub
        'End If
        If Labelid.Text <> "" Then
            '''''''''''''''''''''''''''''
            If GF_storelog1("修改采购品投诉处理") = False Then
                Exit Sub
            End If ' 判断是否有权限
            '''''''''''''''''''''
            ...
            GS_storelog3("修改采购品投诉处理", "修改采购品投诉处理") '储存到日志
        Else
            '''''''''''''''''''''''''''''
            If GF_storelog1("添加采购品投诉处理") = False Then
                Exit Sub
            End If ' 判断是否有权限
            '''''''''''''''''''''
            ...
            GS_storelog3("添加采购品投诉处理", "添加采购品投诉处理") '储存到日志
        End If
        Me.Close()
    End Sub
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        If MsgBox("你确定删除本条记录吗?", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
            ''''''''''''''''''''日志权限判断
            If GF_storelog2("删除采购品投诉处理", "删除采购品投诉处理") = False Then
                Exit Sub
            End If
            '''''''''''''''''''''
