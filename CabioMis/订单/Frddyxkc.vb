Public Class Frddyxkc
    Public dt As DataTable = Nothing
    Private Sub Frddyxkc_Resize(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Resize
        GroupBox1.Height = Me.Height * 3.5 / 4
    End Sub

    Private Sub Frddyxkc_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
        Dim sqlkc2 As String = "select tb_x832pkc_cplb,tb_x832pkc_mingc,tb_x832pkc_jiagdm,sum(tb_x832pkc_ckcl) from tb_x832pkc where tb_x832pkc_sykczl>0 and  tb_x832pkc_qaspjg ='待定' group by tb_x832pkc_cplb,tb_x832pkc_mingc,tb_x832pkc_jiagdm"
        sqlkc2 &= " union all "
        sqlkc2 &= " select tb_h195zkc_cplb,tb_h195zkc_mingc,tb_h195zkc_jiagdm,sum(tb_h195zkc_ckcl)  from tb_h195zkc where tb_h195zkc_sykczl>0 and  tb_h195zkc_qaspjg ='待定' group by tb_h195zkc_cplb,tb_h195zkc_mingc,tb_h195zkc_jiagdm  "
        sqlkc2 &= " union all "
        sqlkc2 &= " select tb_x825pkc_cplb,tb_x825pkc_mingc,tb_x825pkc_jiagdm,sum(tb_x825pkc_ckcl ) from tb_x825pkc where tb_x825pkc_sykczl>0 and  tb_x825pkc_qaspjg ='待定' group by tb_x825pkc_cplb,tb_x825pkc_mingc,tb_x825pkc_jiagdm "
        ...
        If Me.Text = "今日有效库存" And ToolStripLabel1.Text <> "" Then '如果是订单交付月报
            dt.Merge(dtsx(dt1, dt)) '合并没有订单代码的那些产品库存
        End If
        '‘’‘’‘’‘’‘’‘’‘’‘’‘’‘’‘’‘’‘’‘’‘’‘’‘’‘’‘’‘’‘’‘’‘’‘’‘’‘’‘’‘’
        For i As Integer = 0 To dt.Rows.Count - 1
            Try
                ...
                sqlsj &= "where tb_dd_cplx='" & dt.Rows(i)(0) & "' and  tb_dd_cpdm ='" & dt.Rows(i)(1) & "' and tb_dd_jgdm='" & dt.Rows(i)(2) & "'  and tb_dd_fhtjdh is null " & vbCrLf
                sqlsj &= "select @a2 = sum(tb_dd_yfh) from tb_dd " & vbCrLf
                sqlsj &= "where tb_dd_cplx='" & dt.Rows(i)(0) & "' and  tb_dd_cpdm ='" & dt.Rows(i)(1) & "' and tb_dd_jgdm='" & dt.Rows(i)(2) & "'  and tb_dd_fhtjdh is null " & vbCrLf
                sqlsj &= "select @a1,@a2"
                Dim dtt As DataTable = GF_CreateDataSource(G_cnnstr, sqlsj)
                dt.Rows(i)("未发货（暂无计划）") = Null2zero2(dtt.Rows(0)(0)) - Null2zero2(dtt.Rows(0)(1))
                ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                ...
                If ckcl0.Length = 0 Then
                    dt.Rows(i)("有效库存") = Microsoft.VisualBasic.FormatNumber(0 - dt.Rows(i)("未发货（暂无计划）"), 2)
                Else
                    dt.Rows(i)("有效库存") = Microsoft.VisualBasic.FormatNumber(ckcl0(0)(3) - dt.Rows(i)("未发货（暂无计划）"), 2)
                End If
    ...
    End Sub
    Private Function dtsx(ByVal dt1 As DataTable, ByVal dt2 As DataTable) As DataTable 'dt2 为架构,dt1为合格dt
        ...
        For i As Integer = 0 To dt1.Rows.Count - 1
            Try
                ...
                If dtr.Length = 0 Then
                    ...
                    dtr1("产品类型") = dt1.Rows(i)(0)
                    dtr1("产品代码") = dt1.Rows(i)(1)
                    dtr1("加工代码") = dt1.Rows(i)(2)
                    '''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
