...
Public Class FrSelSz
    ...
    Private Sub showViewSet_2(Optional ByVal pSql As String = "")
        Dim sql As String = "select tb_mrp_wpdm,tb_wp_pm,tb_mrp_isbx from tb_mrp as a left join tb_wp as b on a.tb_mrp_wpdm=b.tb_wp_dm  where " &
        ...
        dt.YanDataBind(show2, "tb_mrp_isbx", menu1)
        ...
        End If
    End Sub
    Private Sub ListView1_CellClick(ByVal sender As Object, ByVal e As DataGridViewCellEventArgs) Handles show2.CellClick
        If m_wpfl = en_wpfl.过程品.ToString Or m_wpfl = en_wpfl.成品.ToString Then
    ...
    Private Sub showDbclick_1(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        dt.Rows.Add({dr.Cells("tb_wp_dm").Value, dr.Cells("tb_wp_pm").Value, "否"})
        'lv1删除相对应的行
        ...
        If MsgBox("更新数据吗？", MsgBoxStyle.YesNo) = vbYes Then
            ...
            For Each dr As DataRow In dt.Rows
                If dr.RowState <> DataRowState.Deleted Then
                    sql &= "insert into tb_mrp(tb_mrp_cpbs,tb_mrp_wpdm,tb_mrp_xz,tb_mrp_isbx) " &
                        "values(" & sender.m_iCzBs & ",'" & dr("tb_mrp_wpdm") & "',1,'" & dr("tb_mrp_isbx") & "') " & vbCrLf
                End If
            Next
            If sql <> "" Then
                _D.GS_upztxx(sql)
