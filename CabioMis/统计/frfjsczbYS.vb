...
Public Class frfjsczbYS
    ...
    Public Function getFJSCZBdatatable(ByVal monthStr As String) As Data.DataTable
        ...
        For i As Integer = 2 To monthArr.Length Step 2
            ...
            '''''''''''''''''''''''''''''''''''''''''''''''
            myread.CommandText = "select top 1  tb_x832p_num from tb_x832p where (tb_x832p_jgrq between '" & tmpMonthLow & "' and '" & tmpMonthhigh & "') andtb_x832p_th='1号塔成品粉' order by tb_x832p_jgrq desc "
            ...
            rs.Close()
            ''''''''''''''''''''''''''''''''''1#塔成品批数,1#塔成品产量,
            myread.CommandText = "select count(*),sum(tb_x832p_i232a) from tb_x832p where (tb_x832p_jgrq between '" & tmpMonthLow & "' and '" & tmpMonthhigh & "') andtb_x832p_th='1号塔成品粉'"
            rs = myread.ExecuteReader
            If rs.Read Then
            ...
            rs.Close()
            ''''''''''''''''''''''''''''''菌落计数不是<10和不为空的批数
            myread.CommandText = "select count(*)  from tb_x832p where (tb_x832p_jgrq between '" & tmpMonthLow & "' and '" & tmpMonthhigh & "') andtb_x832p_th='1号塔成品粉'   and tb_x832p_e106x<>'<10' and tb_x832p_e106x is not null"
            rs = myread.ExecuteReader
            If rs.Read Then
                dataRow("1#塔菌落≥10批数") = DeepCode.DeepDoStr.Null2zero(rs(0))
            End If
            rs.Close()
            myread.CommandText = "select count(*)  from tb_x832p where (tb_x832p_jgrq between '" & tmpMonthLow & "' and '" & tmpMonthhigh & "') andtb_x832p_th='34号塔成品粉'   and tb_x832p_e106x<>'<10' and tb_x832p_e106x is not null"
            rs = myread.ExecuteReader
            If rs.Read Then
            ...
            myread.CommandText = "select count(*)  from tb_x832p where (tb_x832p_jgrq between '" & tmpMonthLow & "' and '" & tmpMonthhigh & "') andtb_x832p_th='1号塔成品粉'   and tb_x832p_h199z<>'0/5×10g' and tb_x832p_h199z is not null"
            rs = myread.ExecuteReader
            If rs.Read Then
                dataRow("1#塔成品EB检出批数") = DeepCode.DeepDoStr.Null2zero(rs(0))
            End If
            rs.Close()
            myread.CommandText = "select count(*)  from tb_x832p where (tb_x832p_jgrq between '" & tmpMonthLow & "' and '" & tmpMonthhigh & "') andtb_x832p_th='34号塔成品粉'   and tb_x832p_h199z<>'0/5×10g' and tb_x832p_h199z is not null"
            rs = myread.ExecuteReader
            If rs.Read Then
            ...
            rs.Close()
            ''''''''''''''''''''''''''''''''''''''''''''''''''''''''不平均0
            myread.CommandText = "select avg(tb_x832p_d65v)  from tb_x832p where (tb_x832p_jgrq between '" & tmpMonthLow & "' and '" & tmpMonthhigh & "') andtb_x832p_th='1号塔成品粉'   and tb_x832p_d65v>0"
            rs = myread.ExecuteReader
            If rs.Read Then
            ...
            rs.Close()
            ''''''''''''''''''''''''''''''''不平均0
            myread.CommandText = "select avg(tb_x832p_a948s)  from tb_x832p where (tb_x832p_jgrq between '" & tmpMonthLow & "' and '" & tmpMonthhigh & "') andtb_x832p_th='1号塔成品粉'   and tb_x832p_a948s>0"
            rs = myread.ExecuteReader
            If rs.Read Then
            ...
            rs.Close()
            ''''''''''''''''''''''''''''''''不平均0
            myread.CommandText = "select avg(tb_x832p_i232a)  from tb_x832p where (tb_x832p_jgrq between '" & tmpMonthLow & "' and '" & tmpMonthhigh & "') andtb_x832p_th='1号塔成品粉'   and tb_x832p_i232a>0"
            rs = myread.ExecuteReader
            If rs.Read Then
            ...
            rs.Close()
            ''''''''''''''''''''''''''''''''''''''''''''''''''''''''
            myread.CommandText = "select top 1  tb_x832p_num from tb_x832p where (tb_x832p_jgrq between '" & tmpMonthLow & "' and '" & tmpMonthhigh & "') andtb_x832p_th='34号塔成品粉' order by tb_x832p_jgrq desc "
            rs = myread.ExecuteReader
            If rs.Read Then
            ...
            rs.Close()
            ''''''''''''''''''''''''''''''''''34#塔成品批数,34#塔成品产量
            myread.CommandText = "select count(*),sum(tb_x832p_i232a) from tb_x832p where (tb_x832p_jgrq between '" & tmpMonthLow & "' and '" & tmpMonthhigh & "') andtb_x832p_th='34号塔成品粉'"
            rs = myread.ExecuteReader
            If rs.Read Then
            ...
            rs.Close()
            ''''''''''''''''''''''''''''''''''''''''''''''''''''''''不平均0
            myread.CommandText = "select avg(tb_x832p_d65v)  from tb_x832p where (tb_x832p_jgrq between '" & tmpMonthLow & "' and '" & tmpMonthhigh & "') andtb_x832p_th='34号塔成品粉'   and tb_x832p_d65v>0"
            rs = myread.ExecuteReader
            If rs.Read Then
            ...
            rs.Close()
            ''''''''''''''''''''''''''''''''不平均0
            myread.CommandText = "select avg(tb_x832p_a948s)  from tb_x832p where (tb_x832p_jgrq between '" & tmpMonthLow & "' and '" & tmpMonthhigh & "') andtb_x832p_th='34号塔成品粉'   and tb_x832p_a948s>0"
            rs = myread.ExecuteReader
            If rs.Read Then
            ...
            rs.Close()
            ''''''''''''''''''''''''''''''''不平均0
            myread.CommandText = "select avg(tb_x832p_i232a)  from tb_x832p where (tb_x832p_jgrq between '" & tmpMonthLow & "' and '" & tmpMonthhigh & "') andtb_x832p_th='34号塔成品粉'   and tb_x832p_i232a>0"
            rs = myread.ExecuteReader
            If rs.Read Then
            ...
            myread.CommandText = "select sum(tb_x832p_i232a) from tb_x832p where tb_x832p_jgrq between '" & tmpMonthLow & "' and '" & tmpMonthhigh & "' andtb_x832p_th='1号塔成品粉' and tb_x832p_qaspjg='报废'"
            rs = myread.ExecuteReader
            Dim t1, t2 As Double
            rs.Read()
            ...
            myread.CommandText = "select sum(tb_x832p_i232a) from tb_x832p where tb_x832p_jgrq between '" & tmpMonthLow & "' and '" & tmpMonthhigh & "' andtb_x832p_th='34号塔成品粉' and tb_x832p_qaspjg='报废'"
            rs = myread.ExecuteReader
            ...
            'myread.CommandText = "select sum(tb_x832p_z886r),sum(tb_x832p_i232a),sum(tb_x832p_l349d)  from tb_x832p where tb_x832p_jgrq between '" & tmpMonthLow & "' and '" & tmpMonthhigh & "' andtb_x832p_th='34号塔成品粉'"
            'rs = myread.ExecuteReader
            'If rs.Read Then
            ...
            'End If
            'rs.Close()
            'myread.CommandText = "select sum(tb_x832p_z886r) from tb_x832p where tb_x832p_jgrq between '" & tmpMonthLow & "' and '" & tmpMonthhigh & "' andtb_x832p_th='34号塔成品粉' and tb_x832p_qaspjg='报废'"
            'rs = myread.ExecuteReader
            'If rs.Read Then
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        ''''''''''''''''''''''''''''''''''''''''''''''''''''''
        DeepDoDataTable.DelDataColAtIndex(ylbdb, "1,2,3,4,5,11,12,13,14,15,16,17,18,19")
        ShowLine(Chart1, False, ylbdb, "粉剂一平均指标(周)", DataVisualization.Charting.Docking.Top, ys, 1)
    End Sub
