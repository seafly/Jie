Public Class frfxyjyl2
    Dim xgid, zgid, dgid, myid, cpyid As String
    '发酵油剂原料使用统计
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        Try
            ...
            If dts.Rows.Count > 0 Then
                Call GS_useviewbydb2(Me.ListView1, dts, -1, 0) : Button2.Enabled = False : Exit Sub
            End If
            ''''''''''''''''''''''如果该期报表已有那么直接就显示
            ...
            '''''''''''''''''''''''''''''''''''''''''''''''''''''''''
            sql = "select tb_yxjh_t685m,tb_yxjh_sjzph  from tb_yxjh  where   tb_yxjh_y846q='毛油'  and tb_yxjh_tjq=" & tjq
            dt = GF_CreateDataSource(G_cnnstr, sql)
            If dt.Rows.Count = 0 Then
            ...
            tt541 = frfjsczbYS2.sfyph("tb_x825p", qh341) : tt542 = frfjsczbYS2.sfyph("tb_x825p", qh342)

            If tt541 = "" Or tt542 = "" Then
                MsgBox("毛油起止批号设置不对或尚未生产到计划批次！") : Exit Sub
            End If
            q1 = q1 & " 毛油:" & qh341 : z1 = z1 & " 毛油:" & qh342
            ''''''''''''''''''''''''''''''''''''''
    ...
    Public Function getFJSCZBdatatable(ByVal tjq As String, ByVal phq As String, ByVal phz As String, ByVal sj1 As String, ByVal sj2 As String, ByVal sj21 As String, ByVal sj22 As String, ByVal sj31 As String, ByVal sj32 As String) As Data.DataTable
        ...
        Dim phx4 As String = frfjsczbYS2.ddphzh("select tb_k306c_num from tb_k306c where tb_k306c_jgrq between '" & sj21 & "' and  '" & sj22 & "' ")
        Dim phx5 As String = frfjsczbYS2.ddphzh("select tb_h195z_num from tb_h195z where tb_h195z_jgrq between '" & sj1 & "' and  '" & sj2 & "' ")
        phx1 = GF_zhzfccx(phx1) : phx2 = GF_zhzfccx(phx2) : phx3 = GF_zhzfccx(phx3) : phx4 = GF_zhzfccx(phx4) : phx5 = GF_zhzfccx(phx5)
        Dim id1 As String = frfjsczbYS2.ddphzh("select tb_b970t_ID from tb_b970t where tb_b970t_num in(" & phx1 & ")") '得到对应的所有ID
        Dim id2 As String = frfjsczbYS2.ddphzh("select tb_b959t_ID from tb_b959t where tb_b959t_num in(" & phx2 & ")") '得到对应的所有ID
        Dim id3 As String = frfjsczbYS2.ddphzh("select tb_i234a_ID from tb_i234a where tb_i234a_num in(" & phx3 & ")") '得到对应的所有ID
        Dim id4 As String = frfjsczbYS2.ddphzh("select tb_k306c_ID from tb_k306c where tb_k306c_num in(" & phx4 & ")") '得到对应的所有ID
        Dim id5 As String = frfjsczbYS2.ddphzh("select tb_h195z_ID from tb_h195z where tb_h195z_num in(" & phx5 & ")") '得到对应的所有ID
        ''''''''''''''''''''''''上面得到小中大精炼成品油的批号和ID
        ...
        sql &= "" & fssz("2.06.006") & ",SUM(tb_k306c_j299c)AS '2.06.007'," & fssz("2.06.008") & " ," & fssz("2.06.009") & " ,SUM(tb_k306c_d39v)AS '2.06.010' , " & fssz("2.06.011") & ""
        sql &= " from tb_k306c where tb_k306c_ID in(" & id4 & ")"
        dt1 = GF_CreateDataSource(G_cnnstr, sql)
        nn = dt1.Rows(0) : dt.ImportRow(nn)
        '''''''''''''''''''''''''''''''''''''''''''''''''''得到成品油
        ...
        ''油剂原料的VC棕榈酸脂、活性炭、正丁烷、葵花籽油、二氧化硅、二氧化硅(干胶)
        sql &= "SUM(tb_h195z_b989t)AS '2.06.006'," & fssz("2.06.007") & "," & fssz("2.06.008") & " ,SUM(tb_h195z_p495h)AS '2.06.009' , " & fssz("2.06.010") & ""
        sql &= " from tb_h195z where tb_h195z_ID in(" & id5 & ")"
        dt1 = GF_CreateDataSource(G_cnnstr, sql)
        nn = dt1.Rows(0) : dt.ImportRow(nn)
        ''''''''''''''''''''''建立架构
        ...
        For i As Integer = 0 To dt.Columns.Count - 1
            ...
            For j As Integer = 0 To dt.Rows.Count - 1
                '''''''''''''''''''''''''求和
                hj = FormatNumber((hj + dt.Rows(j)(i)), 2)
            Next
            aaRow(5) = GF_cnwithen(dttemp, "tb_yllb_dm='" & dm & "'", 3)
            aaRow(6) = hj : hj = 0
            aaRow(7) = G_dlrgh : aaRow(8) = id1 : aaRow(9) = id2 : aaRow(10) = id3 : aaRow(11) = id4 : aaRow(12) = id5
    ...
    Private Sub Button2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button2.Click
        ...
        If ccrsdb2(Me.ListView1, "tb_zfxylbb") = True Then
            Call GS_sd("tb_n441f", Me.ListView1.Items(0).SubItems(n - 2).Text, G_dlrgh)
            MsgBox("完成报表数据锁定")
    ...
    Private Function fsszyj() As String '油剂原料
        ...
        '油剂原料的VC棕榈酸脂、活性炭、正丁烷、葵花籽油、二氧化硅、二氧化硅(干胶)
        fsszyj &= "" & fssz("2.06.006") & " , " & fssz("2.06.007") & " ," & fssz("2.06.008") & " ," & fssz("2.06.009") & " ," & fssz("2.06.010") & " "
